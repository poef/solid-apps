...
<div class="solid-contacts" data-flow-list="contacts.view.current">
	<template>
		...
		`},transformers:{firstLetterBlock:function(t,e){let r=t.value.search(/[A-Z]/),n;r==-1?n="#":n=t.value[r].toUpperCase();let i=t.element.closest("[data-flow-list]");if(n!=i.firstLetter){i.firstLetter=n;let s=Pt`<div class="solid-contacts-letter ds-bg-primary">
					${n.toUpperCase()}
				</div>`,o=document.createElement("template");o.innerHTML=s,t.element.closest(".solid-contacts").insertBefore(o.content,t.element.closest(".solid-contacts-entry"))}e(t)}}};var Cc=simply.app({actions:{loadMockContacts:async function(){let t=await this.api.contacts(),e=simply.flow.model({data:t.persons,options:{}});e.addEffect(function(r){return simply.state.effect(()=>r.current.map(n=>{let i=n.last_name.search(/[A-Z]/);return i==-1&&n.last_name.trim()[0].search(/[a-z]/)==-1?n.sort_name="#"+n.last_name:n.sort_name=n.last_name.substring(i).toUpperCase(),n}))}),e.addEffect(simply.flow.sort({sortBy:"sort_name"})),this.state.contacts=e,console.log("contacts",e)}},state:simply.state.signal({path:"/",list:[],solidConfiguration:{client_info:{client_name:"contacts",redirect_uris:[new URL("redirect.html",window.location.href)]},authorize_callback:wc}}),hooks:{start:async function(){simply.bind({root:this.state,transformers:this.transformers}),delete document.body.dataset.swLoading,this.state.webid?.id||await this.actions.webidDialog(),this.hooks.solidPreferencesLoaded.call(this)},solidPreferencesLoaded:async function(){this.actions.loadMockContacts()}},api:K.jsonApi(window.location.href,{contacts:function(){return this.get("../data/contacts.json")}}),components:{webid:cc,solidFS:Lc,solidDrawer:Oc,solidContacts:Bc}});Cc.start();globalThis.contacts=Cc;})();
