...
<div class="solid-contacts" data-flow-map="contacts.view">
	<template>
		...
		`}};var Cc=simply.app({actions:{loadMockContacts:async function(){let t=await this.api.contacts(),e=simply.flow.model({data:t.persons,options:{}});e.addEffect(function(r){return simply.state.effect(()=>r.current.map(n=>{let i=n.last_name.search(/[A-Z]/);return i==-1&&n.last_name.trim()[0].search(/[a-z]/)==-1?n.sort_name="#"+n.last_name:n.sort_name=n.last_name.substring(i).toUpperCase(),n}))}),e.addEffect(simply.flow.sort({sortBy:"sort_name"})),e.addEffect(function(r){let n={};for(let i of r.current){let s=i.sort_name[0];n[s]||(n[s]={capital:s,entries:[]}),n[s].entries.push(i)}return n}),this.state.contacts=e,console.log("contacts",e)}},state:simply.state.signal({path:"/",list:[],solidConfiguration:{client_info:{client_name:"contacts",redirect_uris:[new URL("redirect.html",window.location.href)]},authorize_callback:wc}}),hooks:{start:async function(){simply.bind({root:this.state,transformers:this.transformers}),delete document.body.dataset.swLoading,this.state.webid?.id||await this.actions.webidDialog(),this.hooks.solidPreferencesLoaded.call(this)},solidPreferencesLoaded:async function(){this.actions.loadMockContacts()}},api:K.jsonApi(window.location.href,{contacts:function(){return this.get("../data/contacts.json")}}),components:{webid:cc,solidFS:Lc,solidDrawer:Oc,solidContacts:Bc}});Cc.start();globalThis.contacts=Cc;})();
